...
constexpr uint32_t LROTL(uint32_t x) {
    return ((x) << 8) | ((x) >> 24);
}

constexpr uint32_t LROTR(uint32_t x) {
    return ((x) >> 8) | ((x) << 24);
}

constexpr uint32_t SWAP(uint32_t x) {
    return (LROTL(x) & 0x00ff00ff) | (LROTR(x) & 0xff00ff00);
}

constexpr uint32_t GET(const uint8_t* p) {
    return SWAP(*reinterpret_cast<const uint32_t*>(p));
}

constexpr void PUT(uint8_t* ct, uint32_t st) {
    *reinterpret_cast<uint32_t*>(ct) = SWAP(st);
}
...
static const std::array<uint32_t, 256> Te0 = {
    ...
    0xd86c6cb4, 0xac5656fa, 0xf3f4f407, 0x5c2e2e72, 0x30282870, 0x7c3e3e42, 0x8bcaca41, 0x9b5959eb
};
...
constexpr std::array<std::array<uint8_t, 16>, 16> SBOX = {{
...
public:
    static void aes256_set_encryption_key(const uint8_t key[32], std::array<uint32_t, 60>& expandedKey) {
        uint32_t Nb = 4, Nr = 14, Nk = 8, tmp;
    ...
    static void aes256_set_decryption_key(const uint8_t key[32], std::array<uint32_t, 60>& expandedKey) {
        uint32_t tmp;

        aes256_set_encryption_key(key, expandedKey);

        for (uint32_t i = 0, j = 56; i < j; i += 4, j -= 4) {
...
const std::array<uint32_t, 10> AES256::RCON = {
...
void aes256_encrypt(const uint8_t in[16], uint8_t out[16], const uint32_t key[60]) {
    ...
    for (int i = 8; i < 56; i += 4) {
        s0 = Te0[t0 >> 24] ^ Te1[(t1 >> 16) & 0xff] ^ Te2[(t2 >> 8) & 0xff] ^ Te3[t3 & 0xff] ^ key[i];
        s1 = Te0[t1 >> 24] ^ Te1[(t2 >> 16) & 0xff] ^ Te2[(t3 >> 8) & 0xff] ^ Te3[t0 & 0xff] ^ key[i + 1];
        s2 = Te0[t2 >> 24] ^ Te1[(t3 >> 16) & 0xff] ^ Te2[(t0 >> 8) & 0xff] ^ Te3[t1 & 0xff] ^ key[i + 2];
        s3 = Te0[t3 >> 24] ^ Te1[(t0 >> 16) & 0xff] ^ Te2[(t1 >> 8) & 0xff] ^ Te3[t2 & 0xff] ^ key[i + 3];
